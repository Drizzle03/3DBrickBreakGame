...
#스크린 시점 설정
screen = 0 #1 - right, #2 - left
screen_pos_li = [[-90, 0, 0 ], [-270, 0, 0]]
dir_li = ['RIGHT', 'LEFT']
screen_check = 0 #0 - front, 1 - right or left
...
def draw():
    ...
    # 게임 시작 시
        ...
        global bricks
        global scaleFactor, x,y,z
        global padX, padY, padZ
        global life, screen_check
        checkgameClear()
        checkgameOver()
        ...
        if screen == 1 or screen == 2:
            translate(width / 2 + x + 250, height / 2+y, z) #화면 보정
        else:
...
def draw_end_over_screen():
    ...
    fill(255)
    textAlign(CENTER, CENTER)
    text("Game Over :P", width / 2, height / 2 - 100)
...
def keyPressed():
    ...
    if key == 'F' or key == 'f':
        screen += 1
        screen %= 2
        x = screen_pos_li[screen][0]
        y = screen_pos_li[screen][1]
        z = screen_pos_li[screen][2]
        screen_check = 1
...
def print_info():
    ...
    fill(255)
    textAlign(LEFT)
    text("LIFE : " + str(life), 50, 40)
    
    # 현재 시점
    if screen_check == 1:
        text("Direction : " + str(dir_li[screen]), 50, 80)
    else:
        text("Direction : Front", 50, 80)
    
    text("< Racket Control >", width * 0.8, height * 0.5)
    text("W : Front\nA : Left\nS : Back\nD : Right\nSHIFT : Up\nL-CTRL : Down", width * 0.8, height * 0.55)
    
    text("< Screen Control >", width * 0.8, height * 0.8)
    text("R : Front Screen\nF : Left (one push)\nF : Right (two pushes)", width * 0.8, height * 0.85)
    popMatrix()
